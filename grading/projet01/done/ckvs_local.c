...
// correcteur : c2 a la taille d'un SHA256, si jamais
#define C2_SIZE  32
// ----------------------------------------------------------------------
/**
...
int ckvs_local_getset(const char *filename, const char *key, const char *pwd, const char *set_value) {
    ...
    if (err != ERR_NONE) {
        //error
        ckvs_close(&ckvs); // correcteur : pas besoin de close si le open a fail (dans ce cas ça peut donner du undefined behaviour)
        return err;
    }
    ...
    ckvs_entry_t *ckvs_out; // correcteur : plus simple de juste `= NULL`
    ...
    if (set_value == NULL) {
        ...
        // correcteur : il faudrait check value_len > 0

        //initialize the string where the encrypted secret will be stored
        unsigned char encrypted[ckvs_out->value_len]; // correcteur : VLA, à éviter. calloc à la place, ou pour cette étape on sait que la taille est inférieure à 1200
        ...
        unsigned char decrypted[decrypted_len]; // correcteur : VLA
        ...
        for (size_t i = 0; i < decrypted_len; ++i) {
            // correcteur : pour ce cas les valeurs sont uniquement des fichiers ASCII, mais pas une mauvaise idée en soi
...
//-----------------------------------------------------------------------
// correcteur : pas une mauvause idée, mais c'est des variables locales, un free() suffit
...
int ckvs_local_set(const char *filename, const char *key, const char *pwd, const char *valuefilename) {
    ...
    //free the buffer
    free(buffer);
    buffer = NULL; // correcteur : pas strictement nécessaire puisqu'on return directement (et que les variables sont locales)
    buffer_size = 0;
