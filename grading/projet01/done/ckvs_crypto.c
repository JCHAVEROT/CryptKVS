...
int ckvs_client_encrypt_pwd(ckvs_memrecord_t *mr, const char *key, const char *pwd) {
    ...
    strncat(str, key, strlen(key)); // correcteur : pas de check sur les longueurs des paramètres
    const char slash[1] = "|";
    strncat(str, slash, 1);
    ...
    // correcteur : les deux HMACs devraient être dans une fonction (copié-collé)
